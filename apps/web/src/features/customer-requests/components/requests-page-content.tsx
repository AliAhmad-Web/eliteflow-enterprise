...
export function RequestsPageContent() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="My Requests"
        description="Submit and track work requests for projects, tasks, and services."
        actionLabel={canCreate ? "New request" : undefined}
        onAction={
          canCreate ? () => router.push(ROUTES.REQUESTS_NEW) : undefined
        }
      />
      ...
      <Card className="border-border/50 shadow-(--shadow-sm)">
        <CardContent className="space-y-4 p-6">
          ...
          {showInitialLoading ? (
            <LoadingState label="Loading requests" className="border-0" />
          ) : null}
            ...
            <>
              ...
              {items.length === 0 ? (
                <EmptyState
                  ...
                  description="Start a project or task request and EliteFlow will review it."
                  actionLabel={canCreate ? "New request" : undefined}
                  actionHref={canCreate ? ROUTES.REQUESTS_NEW : undefined}
                />
              ) : (
                  ...
                  {items.map((item) => (
                    <li key={item.id}>
                      <Link
                        ...
                        className="flex flex-col gap-2 p-4 transition-colors hover:bg-accent/40 sm:flex-row sm:items-center sm:justify-between"
                      >
